"""Fail-closed final evidence publication for registered Experiment 002.
...
from falsewake import experiment_002_final_evidence as _completed_evidence
from falsewake import experiment_002_run_authority as _run_authority
...
__all__ = (
    ...
    "Experiment002FinalPublicationError",
    "FailureCode",
    "FailurePhase",
...
_REPORT_PATH: Final = "reports/experiment-002-training.json"
...
_RERUN_HISTORY_PATH: Final = "reports/experiment-002-selected-rerun-history.json"
_MODEL_PATH: Final = "models/experiment-002-selected.safetensors"
...
_TEMP_PREFIX: Final = ".falsewake-exp002-publication-"
...
class Experiment002FinalPublicationError(ValueError):
...
def _fail(message: str) -> NoReturn:
    raise Experiment002FinalPublicationError(message)
...
def _failure_report_bytes(
    ...
    return (
        '{"artifacts":[],"checkpoint_reusable":false,"experiment":"002",'
        ...
        '"schema_version":1,"status":"execution_failure"}\n'
    ).encode("ascii")
...
def _make_failure_evidence_routes() -> tuple[
    ...
    error_type = Experiment002FinalPublicationError
...
def _parse_canonical_report(raw: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        raise Experiment002FinalPublicationError(
...
def _require_report_matches_snapshot(snapshot: FinalPublicationSnapshot) -> None:
    ...
    if (
        ...
        or document.get("experiment") != "002"
    ...
    if status == "execution_failure":
        if document.get("checkpoint_reusable") is not False:
            _fail("execution-failure report could make a checkpoint reusable")
