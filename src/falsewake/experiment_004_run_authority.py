"""Fixed-profile Experiment 004 facade over the shared run authority.
...
route below binds the retained capability to profile ``004`` before delegating.
"""
...
Experiment004RunAuthorityError = _engine.Experiment002RunAuthorityError
...
_SEALED_CHILD_MEMFD_TARGET = _engine._EXPERIMENT_004_SEALED_CHILD_MEMFD_TARGET
...
def verify_and_issue_experiment_004_run_registration() -> VerifiedRunRegistration:
    """Verify the fixed registration and mint the one profile-004 capability."""
...
def _verify_and_issue_experiment_004_sealed_child_registration() -> (
    ...
    """Mint a profile-004 capability from the fixed sealed child bundle."""

    return _engine._verify_and_issue_experiment_004_sealed_child_registration()


def verify_verified_run_registration(
    registration: VerifiedRunRegistration,
) -> None:
    """Fully verify an exact profile-004 capability."""
...
def reverify_verified_run_registration(
    ...
    """Reverify an exact profile-004 capability from its retained state."""
...
def _registered_child_input_snapshot(
    ...
    """Return retained child inputs only for a sealed profile-004 capability."""

    _engine._require_verified_registration_profile(
        registration,
        _engine._EXPERIMENT_004_PROFILE,
    )
...
def _create_sealed_experiment_004_child_bundle_fd(
    ...
    """Create the fixed profile-004 sealed child source-bundle descriptor."""

    return _engine._create_sealed_experiment_004_child_bundle_fd(registration)
