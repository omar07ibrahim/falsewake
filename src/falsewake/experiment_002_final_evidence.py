"""Pure completed-run evidence for the registered Experiment 002 selector.
...
__all__ = (
    ...
    "Experiment002FinalEvidenceError",
...
SELECTED_SAFETENSORS_PATH: Final = "models/experiment-002-selected.safetensors"
FINAL_REPORT_PATH: Final = "reports/experiment-002-training.json"
...
class Experiment002FinalEvidenceError(ValueError):
...
def _fail(message: str) -> NoReturn:
    raise Experiment002FinalEvidenceError(message)
...
def _unique_json_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise Experiment002FinalEvidenceError(
...
def _reject_json_number(value: str) -> NoReturn:
    del value
    raise Experiment002FinalEvidenceError("canonical JSON contains a forbidden number")
...
def _parse_canonical_object(raw: object, maximum: int, name: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise Experiment002FinalEvidenceError(f"{name} is not exact JSON") from error
...
def _require_float_hex(value: object, name: str) -> tuple[str, float]:
    ...
    try:
        ...
        raise Experiment002FinalEvidenceError(f"{name} is not binary64 hex") from error
    if (
...
def _parse_history(raw: object, expected_seed: int) -> _ParsedHistory:
    ...
    _require_string(document["experiment"], _EXPERIMENT, "history experiment")
...
def _parse_envelope(
    ...
    *,
    ...
    _require_string(document["experiment"], _EXPERIMENT, "envelope experiment")
...
def _make_completed_evidence_routes() -> tuple[
    ...
    error_type = Experiment002FinalEvidenceError
    ...
    def bind_build_route(
        ...
        def build_completed_final_evidence(
            ...
            guard_route()
            try:
                result = implementation(selector_result)
            except BaseException:
                guard_route()
                raise
            guard_route()
