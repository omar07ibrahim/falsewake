"""Pure completed-run evidence for the registered Experiment 005 selector.
...
from falsewake.experiment_005_run_authority import VerifiedRunRegistration
...
__all__ = (
    ...
    "Experiment005FinalEvidenceError",
...
SELECTED_SAFETENSORS_PATH: Final = "models/experiment-005-selected.safetensors"
FINAL_REPORT_PATH: Final = "reports/experiment-005-training.json"
...
class Experiment005FinalEvidenceError(ValueError):
...
def _fail(message: str) -> NoReturn:
    raise Experiment005FinalEvidenceError(message)
...
def _unique_json_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise Experiment005FinalEvidenceError(
...
def _reject_json_number(value: str) -> NoReturn:
    del value
    raise Experiment005FinalEvidenceError("canonical JSON contains a forbidden number")
...
def _parse_canonical_object(raw: object, maximum: int, name: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise Experiment005FinalEvidenceError(f"{name} is not exact JSON") from error
...
def _require_float_hex(value: object, name: str) -> tuple[str, float]:
    ...
    try:
        ...
        raise Experiment005FinalEvidenceError(f"{name} is not binary64 hex") from error
    if (
...
def _make_completed_evidence_routes() -> tuple[
    ...
    error_type = Experiment005FinalEvidenceError
    ...
    verified_state_route_authority = capture_function_authority(
        "experiment_005_run_authority._verified_state",
