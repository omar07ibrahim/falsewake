"""Pure completed-run evidence for the registered Experiment 003 selector.
...
from falsewake.experiment_003_run_authority import VerifiedRunRegistration
...
__all__ = (
    ...
    "Experiment003FinalEvidenceError",
...
SELECTED_SAFETENSORS_PATH: Final = "models/experiment-003-selected.safetensors"
FINAL_REPORT_PATH: Final = "reports/experiment-003-training.json"
...
class Experiment003FinalEvidenceError(ValueError):
...
def _fail(message: str) -> NoReturn:
    raise Experiment003FinalEvidenceError(message)
...
def _unique_json_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise Experiment003FinalEvidenceError(
...
def _reject_json_number(value: str) -> NoReturn:
    del value
    raise Experiment003FinalEvidenceError("canonical JSON contains a forbidden number")
...
def _parse_canonical_object(raw: object, maximum: int, name: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise Experiment003FinalEvidenceError(f"{name} is not exact JSON") from error
...
def _require_float_hex(value: object, name: str) -> tuple[str, float]:
    ...
    try:
        ...
        raise Experiment003FinalEvidenceError(f"{name} is not binary64 hex") from error
    if (
...
def _make_completed_evidence_routes() -> tuple[
    ...
    error_type = Experiment003FinalEvidenceError
    ...
    verified_state_route_authority = capture_function_authority(
        "experiment_003_run_authority._verified_state",
