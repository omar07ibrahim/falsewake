"""Exact parent-or-sealed-child bootstrap for Experiment 002.
...
class Experiment002RunnerError(RuntimeError):
    """The exact Experiment 002 bootstrap failed closed."""
...
class _SealedSourceFinder(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    def _require_active(self) -> None:
        if self._failed:
            raise Experiment002RunnerError("sealed source loader is terminally failed")
...
def _classify_execution_role() -> str:
    ...
    try:
        ...
        if error.errno == errno.EBADF:
            return _PARENT_ROLE
        raise Experiment002RunnerError(
...
def _decode_fixed_ascii(value: bytes, name: str) -> str:
    try:
        ...
        raise Experiment002RunnerError(f"{name} is not ASCII") from error
...
def _require_relative_path(value: object, name: str) -> str:
    ...
    try:
        ...
        raise Experiment002RunnerError(f"{name} path is not UTF-8") from error
    if (
        not path
...
def _parse_registration_bindings(
    ...
    *,
    ...
    try:
        ...
        raise Experiment002RunnerError("run registration is not exact JSON") from error
