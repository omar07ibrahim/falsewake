"""Fail-closed process coordination for the registered Experiment 004 run.
...
_WORKER_MODULE: Final = "falsewake.experiment_004_seed_worker"
...
_SUPERVISOR_MODULE: Final = "falsewake.experiment_004_supervisor"
...
_FINAL_PUBLICATION_MODULE: Final = "falsewake.experiment_004_final_publication"
...
_TICKET_DIGEST_DOMAIN: Final = b"falsewake-exp004-activation-ticket-v1\0"
...
class Experiment004CoordinatorError(RuntimeError):
    """Experiment 004 process coordination failed closed."""
...
class _RegisteredExperimentBoundaryError(Experiment004CoordinatorError):
...
def _make_trusted_protocol_ledger() -> tuple[
    ...
    def reserve_issued(nonce: str, digest: str) -> None:
        with ledger_lock:
            if nonce in issued_nonces or digest in issued_digests:
                raise Experiment004CoordinatorError(
    ...
    def accept_ticket(ticket: _Ticket) -> _AcceptedTicketReceipt:
        with ledger_lock:
            ...
            if accepted_nonce in accepted_nonces or accepted_digest in accepted_digests:
                raise Experiment004CoordinatorError("activation ticket was replayed")
    ...
    def record_issuance(
        ...
        with ledger_lock:
            ...
            except (TypeError, Experiment004CoordinatorError):
                current_frame = None
            if (
...
def _require_registration_binding(binding: _RegistrationBinding) -> None:
    if type(binding) is not _RegistrationBinding:
        raise Experiment004CoordinatorError("registration binding has an invalid type")
...
def _snapshot_registered_cpu_ids(cpu_ids: tuple[int, int], /) -> tuple[int, int]:
    if (
        ...
        raise Experiment004CoordinatorError("captured CPU IDs are invalid")
...
def _assignment(role: object, seed: object) -> _Assignment:
    ...
    raise Experiment004CoordinatorError("child role and seed are not registered")
...
def _encode_packet(document: dict[str, object]) -> bytes:
    ...
    if not payload or len(payload) > _MAX_JSON_BYTES:
        raise Experiment004CoordinatorError("protocol JSON size is invalid")
...
def _decode_packet(packet: bytes) -> dict[str, Any]:
    ...
    if not _FRAME_HEADER.size < len(packet) <= _MAX_PACKET_BYTES:
        raise Experiment004CoordinatorError("protocol packet size is invalid")
    ...
    if magic != _FRAME_MAGIC or payload_length != len(payload):
        raise Experiment004CoordinatorError("protocol frame header is invalid")
    try:
    ...
    if type(parsed) is not dict:
        raise Experiment004CoordinatorError("protocol payload must be an object")
    document = cast(dict[str, Any], parsed)
    if _encode_packet(cast(dict[str, object], document)) != packet:
        raise Experiment004CoordinatorError("protocol payload is not canonical")
    return document
...
def _send_packet_prepared(channel: socket.socket, packet: bytes) -> None:
    ...
    if type(packet) is not bytes or not 0 < len(packet) <= _MAX_PACKET_BYTES:
        raise Experiment004CoordinatorError("outbound protocol packet is invalid")
    ancillary = [
        (
            socket.SOL_SOCKET,
    ...
    try:
        ...
        raise Experiment004CoordinatorError("protocol packet send failed") from error
    if type(sent) is not int or sent != len(packet):
        raise Experiment004CoordinatorError("protocol packet send was incomplete")
...
def _receive_packet_frame(
    ...
    try:
        ...
        raise Experiment004CoordinatorError("protocol packet receive failed") from error
    ...
    if flags != _RECEIVE_CLOEXEC:
        raise Experiment004CoordinatorError("protocol receive flags are invalid")
    if received_rights:
        raise Experiment004CoordinatorError(
    ...
    if len(packet) > _MAX_PACKET_BYTES:
        raise Experiment004CoordinatorError("protocol packet exceeds its maximum size")
    ...
    if len(payload) != _CREDENTIALS.size:
        raise Experiment004CoordinatorError("SCM_CREDENTIALS has an invalid size")
...
def _require_seqpacket_channel(channel: socket.socket) -> None:
    ...
    if (
        ...
        raise Experiment004CoordinatorError("Linux SCM_CREDENTIALS support is required")
    if channel.family is not socket.AF_UNIX:
        raise Experiment004CoordinatorError("protocol channel must use AF_UNIX")
    try:
...
def _prepare_send_channel(channel: socket.socket) -> None:
    ...
    try:
        ...
        raise Experiment004CoordinatorError("protocol channel setup failed") from error
    timeout = channel.gettimeout()
    if type(timeout) is not float or timeout != _PROTOCOL_TIMEOUT_SECONDS:
        raise Experiment004CoordinatorError(
            "outbound protocol channel timeout is not exact"
...
def _require_prepared_receive_channel(channel: socket.socket) -> None:
    ...
    if (
        ...
        raise Experiment004CoordinatorError("protocol channel options are not exact")
...
def _require_local_identity(credentials: _PeerCredentials) -> None:
    if type(credentials) is not _PeerCredentials:
        raise Experiment004CoordinatorError("peer credentials have an invalid type")
    if (
        ...
        raise Experiment004CoordinatorError("protocol sender identity is invalid")
...
def _make_verified_child_result_snapshot_frame() -> Callable[
    ...
    error_type = Experiment004CoordinatorError
...
def _run_one_registered_parent_child(
    ...
    try:
        ...
        try:
            ...
            cause = (
                primary.__cause__
                if type(primary) is Experiment004CoordinatorError
...
def _make_registered_experiment_route() -> Callable[
    ...
    def run_registered_experiment(
        ...
        """Run and publish the sole fixed registered Experiment 004 attempt.
...
def _validate_ticket(
    ...
    *,
    ...
    if type(document["kind"]) is not str or document["kind"] != "activation_ticket":
        raise Experiment004CoordinatorError("activation ticket kind is invalid")
    ...
    if (
        ...
        raise Experiment004CoordinatorError("activation ticket ordinal is invalid")
...
def _parse_binding(value: object) -> _RegistrationBinding:
    if type(value) is not dict:
        raise Experiment004CoordinatorError("ticket registration must be an object")
...
def _require_exact_keys(
    ...
    if type(document) is not dict:
        raise Experiment004CoordinatorError(f"{name} fields are not exact")
    if any(type(key) is not str for key in document):
        raise Experiment004CoordinatorError(f"{name} keys must be strings")
    if type(expected) is not set or any(type(key) is not str for key in expected):
        raise Experiment004CoordinatorError(f"{name} expected fields are invalid")
...
def _require_schema_version(value: object) -> None:
    if type(value) is not int or value != _SCHEMA_VERSION:
        raise Experiment004CoordinatorError("protocol schema version is invalid")
...
def _require_pid(value: object, name: str) -> int:
    ...
    if value < 1:
        raise Experiment004CoordinatorError(f"{name} must be positive")
    return value
...
def _require_distinct_processes(parent_pid: object, child_pid: object) -> None:
    ...
    if parent == child:
        raise Experiment004CoordinatorError("parent and child PIDs must be distinct")
...
def _require_lower_hex(value: object, length: int, name: str) -> None:
    if (
        ...
        raise Experiment004CoordinatorError(f"{name} must be {length} lowercase hex")


def _require_assignment(assignment: _Assignment) -> None:
    if type(assignment) is not _Assignment:
        raise Experiment004CoordinatorError("child assignment has an invalid type")
    if (
    ...
    if not _assignments_match(assignment, expected):
        raise Experiment004CoordinatorError("child assignment is not registered")
...
def _require_ticket(ticket: _Ticket) -> None:
    if type(ticket) is not _Ticket:
        raise Experiment004CoordinatorError("activation ticket has an invalid type")
    ...
    *,
    ...
    with _PROTOCOL_LOCK:
        ...
        if nonce in _ISSUED_TICKET_NONCES or packet_sha256 in _ISSUED_TICKET_DIGESTS:
            raise Experiment004CoordinatorError("activation ticket was already issued")
