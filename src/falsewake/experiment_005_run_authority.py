"""Fixed-profile Experiment 005 facade over the shared run authority.
...
route below binds the retained capability to profile ``005`` before delegating.
"""
...
Experiment005RunAuthorityError = _engine.Experiment002RunAuthorityError
...
_SEALED_CHILD_MEMFD_TARGET = _engine._EXPERIMENT_005_SEALED_CHILD_MEMFD_TARGET
...
def verify_and_issue_experiment_005_run_registration() -> VerifiedRunRegistration:
    """Verify the fixed registration and mint the one profile-005 capability."""
...
def _verify_and_issue_experiment_005_sealed_child_registration() -> (
    ...
    """Mint a profile-005 capability from the fixed sealed child bundle."""

    return _engine._verify_and_issue_experiment_005_sealed_child_registration()


def verify_verified_run_registration(
    registration: VerifiedRunRegistration,
) -> None:
    """Fully verify an exact profile-005 capability."""
...
def reverify_verified_run_registration(
    ...
    """Reverify an exact profile-005 capability from its retained state."""
...
def _registered_child_input_snapshot(
    ...
    """Return retained child inputs only for a sealed profile-005 capability."""

    _engine._require_verified_registration_profile(
        registration,
        _engine._EXPERIMENT_005_PROFILE,
    )
...
def _create_sealed_experiment_005_child_bundle_fd(
    ...
    """Create the fixed profile-005 sealed child source-bundle descriptor."""

    return _engine._create_sealed_experiment_005_child_bundle_fd(registration)
