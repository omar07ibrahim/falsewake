"""Fail-closed parent resource kernel for Experiment 006 children.
...
__all__ = ("Experiment006SupervisorError",)

_REPOSITORY_ROOT: Final = "/home/ubuntu/gitcode/falsewake"
_TEMP_ROOT: Final = "/home/ubuntu/gitcode/.t"
_SCRATCH_ROOT: Final = f"{_TEMP_ROOT}/falsewake-experiment-006-scratch"
_STAGING_ROOT: Final = f"{_TEMP_ROOT}/falsewake-experiment-006-staging"
...
_RUNNER_ENTRYPOINT: Final = "src/falsewake/experiment_006_runner.py"
...
class Experiment006SupervisorError(RuntimeError):
...
def _fail(message: str) -> NoReturn:
    raise Experiment006SupervisorError(message)
...
def _open_validated_regular_path(
    ...
    *,
    ...
    try:
        ...
        raise Experiment006SupervisorError(
    ...
    *,
    ...
    try:
        snapshot = os.memfd_create(
            "falsewake-experiment-006-launch",
            _LAUNCH_MEMFD_FLAGS,
        )
        os.lseek(descriptor, 0, os.SEEK_SET)
...
def _validate_launch_surface(plan: _LaunchPlan) -> None:
    ...
    try:
        ...
        raise Experiment006SupervisorError("cannot inspect the parent cwd") from error
...
def _require_existing_output_root(path: str) -> None:
    ...
    except Experiment006SupervisorError as error:
        raise Experiment006SupervisorError(
...
def _scan_directory(
    ...
    *,
    ...
    try:
        ...
        raise Experiment006SupervisorError("output name is not valid UTF-8") from error
    for name in ordered:
        entries_seen[0] += 1
...
def _parse_task_vmrss_bytes(contents: bytes) -> int | None:
    ...
    except Experiment006SupervisorError:
        if (
            type(contents) is not bytes
            or not contents
            or len(contents) > _PROC_FILE_BYTES_MAXIMUM
...
def _read_process_sample(pid: int) -> _ProcessSample:
    ...
    try:
        ...
        raise Experiment006SupervisorError("cannot enumerate child tasks") from error
    tids: list[int] = []
    for name in names:
        if (
...
def _require_libc_success(result: object, operation: str) -> None:
    ...
    if result != 0:
        raise Experiment006SupervisorError(f"{operation} failed") from OSError(
            result,
            os.strerror(result),
        )
...
def _require_single_parent_thread() -> None:
    try:
        ...
        raise Experiment006SupervisorError("cannot enumerate parent tasks") from error
...
def _start_activation(
    ...
    thread = threading.Thread(
        target=activate,
        name="falsewake-exp006-child-activation",
        daemon=True,
    )
    thread.start()
    return state, thread
...
def _bind_registered_parent_lifecycle() -> tuple[
    ...
    error_type = Experiment006SupervisorError
