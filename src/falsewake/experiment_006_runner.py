"""Exact parent-or-sealed-child bootstrap for Experiment 006.
...
_ENTRYPOINT: Final = "src/falsewake/experiment_006_runner.py"
...
_EXPECTED_ENVIRONMENT: Final = {
    ...
    "PYTHONHASHSEED": "0",
    "PYTHONNOUSERSITE": "1",
    "TMPDIR": "/home/ubuntu/gitcode/.t/falsewake-experiment-006-scratch",
    "TZ": "UTC",
}
...
_AUTHORITY_MODULE: Final = "falsewake.experiment_006_run_authority"
_COORDINATOR_MODULE: Final = "falsewake.experiment_006_coordinator"
...
_ATTEMPT_MARKER_DIRECTORY: Final = "/home/ubuntu/gitcode/.t"
_ATTEMPT_MARKER_LEAF: Final = "falsewake-experiment-006-attempt"
_ATTEMPT_MARKER_PATH: Final = f"{_ATTEMPT_MARKER_DIRECTORY}/{_ATTEMPT_MARKER_LEAF}"
_ATTEMPT_MARKER_BYTES: Final = b"falsewake-experiment-006-attempt-v1\n"
_ATTEMPT_MARKER_CREATION_MODE: Final = 0o600
_ATTEMPT_MARKER_FINAL_MODE: Final = 0o444
_CHILD_BUNDLE_MAGIC: Final = b"FW6CHLD1"
_CHILD_BUNDLE_VERSION: Final = 1
_CHILD_BUNDLE_HEADER: Final = struct.Struct("<8sI40s40s64s64sIQQ")
_SOURCE_BUNDLE_DOMAIN: Final = b"falsewake-exp006-source-bundle-v1\0"
...
_SEALED_ORIGIN_PREFIX: Final = "falsewake-sealed://experiment-006"
...
class Experiment006RunnerError(RuntimeError):
    """The exact Experiment 006 bootstrap failed closed."""
...
class _SealedSourceFinder(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    def _poison(self, message: str) -> NoReturn:
        self._failed = True
        raise Experiment006RunnerError(message)

    def _require_active(self) -> None:
        if self._failed:
            raise Experiment006RunnerError("sealed source loader is terminally failed")
...
def _classify_execution_role() -> str:
    ...
    try:
        ...
        if error.errno == errno.EBADF:
            return _PARENT_ROLE
        raise Experiment006RunnerError(
...
def _claim_permanent_parent_attempt() -> None:
    """Consume the sole parent attempt before importing project code."""
    ...
    try:
        expected_marker_path = f"{_ATTEMPT_MARKER_DIRECTORY}/{_ATTEMPT_MARKER_LEAF}"
        if (
            type(_ATTEMPT_MARKER_DIRECTORY) is not str
            or type(_ATTEMPT_MARKER_LEAF) is not str
            or type(_ATTEMPT_MARKER_PATH) is not str
            or type(_ATTEMPT_MARKER_BYTES) is not bytes
            or expected_marker_path != _ATTEMPT_MARKER_PATH
            or not _ATTEMPT_MARKER_BYTES
            or "/" in _ATTEMPT_MARKER_LEAF
            or _ATTEMPT_MARKER_LEAF in {".", ".."}
        ):
            _fail("permanent attempt-marker constants are invalid")
        try:
            ...
            raise Experiment006RunnerError(
                "permanent attempt-marker directory is inaccessible"
            ) from error
        if type(opened_directory) is not int or opened_directory < 0:
            _fail("permanent attempt-marker directory descriptor is invalid")
        directory_descriptor = opened_directory
        try:
            directory_before = os.fstat(directory_descriptor)
            directory_flags = fcntl.fcntl(directory_descriptor, fcntl.F_GETFD)
            directory_target = os.readlink(f"/proc/self/fd/{directory_descriptor}")
        except (OSError, ValueError) as error:
            raise Experiment006RunnerError(
                "permanent attempt-marker directory cannot be inspected"
            ) from error
        directory_identity = (
        ...
        if (
            ...
            or stat.S_IMODE(directory_before.st_mode) & 0o002
            or type(directory_flags) is not int
            or directory_flags != fcntl.FD_CLOEXEC
            or os.get_inheritable(directory_descriptor) is not False
            or type(directory_target) is not str
            or directory_target != _ATTEMPT_MARKER_DIRECTORY
        ):
            _fail("permanent attempt-marker directory identity is invalid")
        try:
            opened_marker = os.open(
            ...
            raise Experiment006RunnerError(
                "the permanent Experiment 006 attempt is unavailable"
            ) from error
        if type(opened_marker) is not int or opened_marker < 0:
            _fail("permanent attempt-marker descriptor is invalid")
        marker_descriptor = opened_marker

        try:
            marker_initial = os.fstat(marker_descriptor)
            marker_flags = fcntl.fcntl(marker_descriptor, fcntl.F_GETFD)
            marker_status = fcntl.fcntl(marker_descriptor, fcntl.F_GETFL)
        except (OSError, ValueError) as error:
            raise Experiment006RunnerError(
                "permanent attempt marker cannot be inspected"
            ) from error
        if (
            not stat.S_ISREG(marker_initial.st_mode)
            or marker_initial.st_nlink != 1
            or marker_initial.st_uid != os.geteuid()
            or marker_initial.st_gid != os.getegid()
            or stat.S_IMODE(marker_initial.st_mode) != _ATTEMPT_MARKER_CREATION_MODE
            or marker_initial.st_size != 0
            or type(marker_flags) is not int
            or marker_flags != fcntl.FD_CLOEXEC
            or type(marker_status) is not int
            or marker_status & os.O_ACCMODE != os.O_WRONLY
            or os.get_inheritable(marker_descriptor) is not False
        ):
            _fail("new permanent attempt marker is not exact")

        offset = 0
        while offset < len(_ATTEMPT_MARKER_BYTES):
            try:
                written = os.write(
                    marker_descriptor,
                    _ATTEMPT_MARKER_BYTES[offset:],
                )
            except InterruptedError:
                continue
            except OSError as error:
                raise Experiment006RunnerError(
                    "permanent attempt-marker write failed"
                ) from error
            if type(written) is not int or written < 1:
                _fail("permanent attempt-marker write made no progress")
            offset += written
        try:
            os.fchmod(marker_descriptor, _ATTEMPT_MARKER_FINAL_MODE)
            os.fsync(marker_descriptor)
            os.fsync(directory_descriptor)
            marker_final = os.fstat(marker_descriptor)
            ...
            final_directory_target = os.readlink(
                f"/proc/self/fd/{directory_descriptor}"
            )
        except (OSError, ValueError) as error:
            raise Experiment006RunnerError(
                "permanent attempt marker could not be finalized"
            ) from error
        ...
        final_marker_identity = (
            marker_final.st_dev,
            marker_final.st_ino,
            marker_final.st_mode,
            marker_final.st_nlink,
            marker_final.st_uid,
            marker_final.st_gid,
            marker_final.st_size,
        )
        named_marker_identity = (
            marker_named.st_dev,
            marker_named.st_ino,
            marker_named.st_mode,
            marker_named.st_nlink,
            marker_named.st_uid,
            marker_named.st_gid,
            marker_named.st_size,
        )
        if (
            final_directory_identity != directory_identity
            or final_directory_target != _ATTEMPT_MARKER_DIRECTORY
            or not stat.S_ISREG(marker_final.st_mode)
            or marker_final.st_nlink != 1
            or marker_final.st_uid != os.geteuid()
            or marker_final.st_gid != os.getegid()
            or stat.S_IMODE(marker_final.st_mode) != _ATTEMPT_MARKER_FINAL_MODE
            or marker_final.st_size != len(_ATTEMPT_MARKER_BYTES)
            or os.lseek(marker_descriptor, 0, os.SEEK_CUR) != len(_ATTEMPT_MARKER_BYTES)
            or named_marker_identity != final_marker_identity
        ):
            _fail("permanent attempt marker changed while being finalized")
        try:
            opened_reader = os.open(
                _ATTEMPT_MARKER_LEAF,
            ...
            raise Experiment006RunnerError(
                "permanent attempt marker cannot be reopened"
            ) from error
        if type(opened_reader) is not int or opened_reader < 0:
            _fail("permanent attempt-marker reader is invalid")
        reader_descriptor = opened_reader
        try:
            reader_frame = os.fstat(reader_descriptor)
            reader_flags = fcntl.fcntl(reader_descriptor, fcntl.F_GETFD)
            reader_status = fcntl.fcntl(reader_descriptor, fcntl.F_GETFL)
            payload = os.pread(
                reader_descriptor,
                len(_ATTEMPT_MARKER_BYTES),
                0,
            )
            trailing = os.pread(
                reader_descriptor,
            ...
            raise Experiment006RunnerError(
                "permanent attempt marker cannot be verified"
            ) from error
        reader_identity = (
            reader_frame.st_dev,
            reader_frame.st_ino,
            reader_frame.st_mode,
            reader_frame.st_nlink,
            reader_frame.st_uid,
            reader_frame.st_gid,
            reader_frame.st_size,
        )
        if (
            reader_identity != final_marker_identity
            or type(reader_flags) is not int
            or reader_flags != fcntl.FD_CLOEXEC
            or type(reader_status) is not int
            or reader_status & os.O_ACCMODE != os.O_RDONLY
            or os.get_inheritable(reader_descriptor) is not False
            or payload != _ATTEMPT_MARKER_BYTES
            or trailing != b""
        ):
            _fail("permanent attempt-marker bytes are not exact")
        ...
        if primary is None and close_failures:
            primary = Experiment006RunnerError(
                "permanent attempt-marker descriptor close failed"
            )
    if primary is not None:
        if type(primary) is Experiment006RunnerError:
            raise primary
        raise Experiment006RunnerError(
            "permanent Experiment 006 attempt claim failed"
        ) from primary
...
def _decode_fixed_ascii(value: bytes, name: str) -> str:
    try:
        ...
        raise Experiment006RunnerError(f"{name} is not ASCII") from error
...
def _require_relative_path(value: object, name: str) -> str:
    ...
    try:
        ...
        raise Experiment006RunnerError(f"{name} path is not UTF-8") from error
    if (
        not path
...
def _parse_registration_bindings(
    ...
    *,
    ...
    try:
        ...
        raise Experiment006RunnerError("run registration is not exact JSON") from error
    ...
    if type(document["experiment"]) is not str or document["experiment"] != "006":
