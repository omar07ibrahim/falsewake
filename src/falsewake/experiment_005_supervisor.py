"""Fail-closed parent resource kernel for Experiment 005 children.
...
__all__ = ("Experiment005SupervisorError",)

_REPOSITORY_ROOT: Final = "/home/ubuntu/gitcode/falsewake"
_TEMP_ROOT: Final = "/home/ubuntu/gitcode/.t"
_SCRATCH_ROOT: Final = f"{_TEMP_ROOT}/falsewake-experiment-005-scratch"
_STAGING_ROOT: Final = f"{_TEMP_ROOT}/falsewake-experiment-005-staging"
...
_RUNNER_ENTRYPOINT: Final = "src/falsewake/experiment_005_runner.py"
...
class Experiment005SupervisorError(RuntimeError):
...
def _fail(message: str) -> NoReturn:
    raise Experiment005SupervisorError(message)
...
def _open_validated_regular_path(
    ...
    *,
    ...
    try:
        ...
        raise Experiment005SupervisorError(
    ...
    *,
    ...
    try:
        snapshot = os.memfd_create(
            "falsewake-experiment-005-launch",
            _LAUNCH_MEMFD_FLAGS,
        )
        os.lseek(descriptor, 0, os.SEEK_SET)
...
def _validate_launch_surface(plan: _LaunchPlan) -> None:
    ...
    try:
        ...
        raise Experiment005SupervisorError("cannot inspect the parent cwd") from error
...
def _require_existing_output_root(path: str) -> None:
    ...
    except Experiment005SupervisorError as error:
        raise Experiment005SupervisorError(
...
def _scan_directory(
    ...
    *,
    ...
    try:
        ...
        raise Experiment005SupervisorError("output name is not valid UTF-8") from error
    for name in ordered:
        entries_seen[0] += 1
...
def _parse_task_vmrss_bytes(contents: bytes) -> int | None:
    ...
    except Experiment005SupervisorError:
        if (
            type(contents) is not bytes
            or not contents
            or len(contents) > _PROC_FILE_BYTES_MAXIMUM
...
def _read_process_sample(pid: int) -> _ProcessSample:
    ...
    try:
        ...
        raise Experiment005SupervisorError("cannot enumerate child tasks") from error
    tids: list[int] = []
    for name in names:
        if (
...
def _require_libc_success(result: object, operation: str) -> None:
    ...
    if result != 0:
        raise Experiment005SupervisorError(f"{operation} failed") from OSError(
            result,
            os.strerror(result),
        )
...
def _require_single_parent_thread() -> None:
    try:
        ...
        raise Experiment005SupervisorError("cannot enumerate parent tasks") from error
...
def _start_activation(
    ...
    thread = threading.Thread(
        target=activate,
        name="falsewake-exp005-child-activation",
        daemon=True,
    )
    thread.start()
    return state, thread
...
def _bind_registered_parent_lifecycle() -> tuple[
    ...
    error_type = Experiment005SupervisorError
