"""Exact parent-or-sealed-child bootstrap for Experiment 005.
...
_ENTRYPOINT: Final = "src/falsewake/experiment_005_runner.py"
...
_EXPECTED_ENVIRONMENT: Final = {
    ...
    "PYTHONHASHSEED": "0",
    "PYTHONNOUSERSITE": "1",
    "TMPDIR": "/home/ubuntu/gitcode/.t/falsewake-experiment-005-scratch",
    "TZ": "UTC",
}
...
_AUTHORITY_MODULE: Final = "falsewake.experiment_005_run_authority"
_COORDINATOR_MODULE: Final = "falsewake.experiment_005_coordinator"
...
_SEALED_ORIGIN_PREFIX: Final = "falsewake-sealed://experiment-005"
...
class Experiment005RunnerError(RuntimeError):
    """The exact Experiment 005 bootstrap failed closed."""
...
class _SealedSourceFinder(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    def _poison(self, message: str) -> NoReturn:
        self._failed = True
        raise Experiment005RunnerError(message)

    def _require_active(self) -> None:
        if self._failed:
            raise Experiment005RunnerError("sealed source loader is terminally failed")
...
def _classify_execution_role() -> str:
    ...
    try:
        ...
        if error.errno == errno.EBADF:
            return _PARENT_ROLE
        raise Experiment005RunnerError(
...
def _claim_permanent_parent_attempt() -> None:
    ...
    try:
        ...
        try:
            ...
            raise Experiment005RunnerError(
                "the permanent Experiment 005 attempt is unavailable"
            ) from error
        ...
        if primary is None and close_failures:
            primary = Experiment005RunnerError(
                "permanent attempt-marker descriptor close failed"
            )
    if primary is not None:
        if type(primary) is Experiment005RunnerError:
            raise primary
        raise Experiment005RunnerError(
            "permanent Experiment 005 attempt claim failed"
        ) from primary
...
def _decode_fixed_ascii(value: bytes, name: str) -> str:
    try:
        ...
        raise Experiment005RunnerError(f"{name} is not ASCII") from error
...
def _require_relative_path(value: object, name: str) -> str:
    ...
    try:
        ...
        raise Experiment005RunnerError(f"{name} path is not UTF-8") from error
    if (
        not path
...
def _parse_registration_bindings(
    ...
    *,
    ...
    try:
        ...
        raise Experiment005RunnerError("run registration is not exact JSON") from error
    ...
    if type(document["experiment"]) is not str or document["experiment"] != "005":
