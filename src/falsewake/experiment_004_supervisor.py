"""Fail-closed parent resource kernel for Experiment 004 children.
...
__all__ = ("Experiment004SupervisorError",)

_REPOSITORY_ROOT: Final = "/home/ubuntu/gitcode/falsewake"
_TEMP_ROOT: Final = "/home/ubuntu/gitcode/.t"
_SCRATCH_ROOT: Final = f"{_TEMP_ROOT}/falsewake-experiment-004-scratch"
_STAGING_ROOT: Final = f"{_TEMP_ROOT}/falsewake-experiment-004-staging"
...
_RUNNER_ENTRYPOINT: Final = "src/falsewake/experiment_004_runner.py"
...
class Experiment004SupervisorError(RuntimeError):
...
def _fail(message: str) -> NoReturn:
    raise Experiment004SupervisorError(message)
...
def _open_validated_regular_path(
    ...
    *,
    ...
    try:
        ...
        raise Experiment004SupervisorError(
    ...
    *,
    ...
    try:
        snapshot = os.memfd_create(
            "falsewake-experiment-004-launch",
            _LAUNCH_MEMFD_FLAGS,
        )
        os.lseek(descriptor, 0, os.SEEK_SET)
...
def _validate_launch_surface(plan: _LaunchPlan) -> None:
    ...
    try:
        ...
        raise Experiment004SupervisorError("cannot inspect the parent cwd") from error
...
def _require_existing_output_root(path: str) -> None:
    ...
    except Experiment004SupervisorError as error:
        raise Experiment004SupervisorError(
...
def _scan_directory(
    ...
    *,
    ...
    try:
        ...
        raise Experiment004SupervisorError("output name is not valid UTF-8") from error
    for name in ordered:
        entries_seen[0] += 1
...
def _parse_task_vmrss_bytes(contents: bytes) -> int | None:
    ...
    except Experiment004SupervisorError:
        if (
            type(contents) is not bytes
            or not contents
            or len(contents) > _PROC_FILE_BYTES_MAXIMUM
...
def _read_process_sample(pid: int) -> _ProcessSample:
    ...
    try:
        ...
        raise Experiment004SupervisorError("cannot enumerate child tasks") from error
    tids: list[int] = []
    for name in names:
        if (
...
def _require_libc_success(result: object, operation: str) -> None:
    ...
    if result != 0:
        raise Experiment004SupervisorError(f"{operation} failed") from OSError(
            result,
            os.strerror(result),
        )
...
def _require_single_parent_thread() -> None:
    try:
        ...
        raise Experiment004SupervisorError("cannot enumerate parent tasks") from error
...
def _start_activation(
    ...
    thread = threading.Thread(
        target=activate,
        name="falsewake-exp004-child-activation",
        daemon=True,
    )
    thread.start()
    return state, thread
...
def _bind_registered_parent_lifecycle() -> tuple[
    ...
    error_type = Experiment004SupervisorError
