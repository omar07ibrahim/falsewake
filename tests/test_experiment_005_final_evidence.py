...
import falsewake.experiment_005_final_evidence as final_evidence
...
_REPORT_SHA256 = {
    "pass": "397efd8138ea4a5bd78bc4789363dfb2b9a02419ed019c8ac8ddc3a4b8f4eee7",
    "gate_failure": "88eb68b2052d025bfb22b35457496fe1d207bf431df35428ccafe6ad4a64a8bd",
}
...
def _profile_004_registration() -> Iterator[None]:
    ...
    snapshot = authority_engine._RepositorySnapshot(
        ...
        profile=authority_engine._EXPERIMENT_005_PROFILE,
    )
...
def test_surface_is_standard_library_only_fixed_and_defaultless() -> None:
    source_path = Path("src/falsewake/experiment_005_final_evidence.py")
    ...
    assert final_evidence.FINAL_REPORT_PATH == "reports/experiment-005-training.json"
...
def test_builder_rejects_cross_profile_and_mismatched_live_registration() -> None:
    ...
    mismatched_registration = issue(
        authority_engine._EXPERIMENT_005_PROFILE,
        head_commit="e" * 40,
    )
    with pytest.raises(
        final_evidence.Experiment005FinalEvidenceError,
        match="differs from the live Experiment 005 capability",
    ):
        final_evidence.build_completed_final_evidence(
...
def test_pass_issues_exact_evidence_artifacts_resources_and_canonical_report() -> None:
    ...
    assert evidence.experiment == "005"
    ...
    assert report_document["experiment"] == "005"
...
def test_selector_result_shape_selection_and_process_identity_fail_closed(
    ...
    with pytest.raises((TypeError, final_evidence.Experiment005FinalEvidenceError)):
...
def test_training_role_ordinal_order_and_common_cpu_binding_are_exact() -> None:
    ...
    with pytest.raises(final_evidence.Experiment005FinalEvidenceError):
