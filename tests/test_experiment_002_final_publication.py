...
import falsewake.experiment_002_final_publication as publication
...
_MODEL_PATH = "models/experiment-002-selected.safetensors"
_REPORT_PATH = "reports/experiment-002-training.json"
_MANAGED_PATHS = (*_HISTORY_PATHS, _MODEL_PATH, _REPORT_PATH)
_TEMP_PREFIX = ".falsewake-exp002-publication-"
...
def test_public_route_closure_rejects_constant_os_and_class_tampering(
    ...
    with monkeypatch.context() as scoped:
        ...
        with pytest.raises(
            publication.Experiment002FinalPublicationError, match="routes changed"
        ):
            require_routes()
    require_routes()
    ...
    try:
        with pytest.raises(
            publication.Experiment002FinalPublicationError,
...
def test_unregistered_failure_pair_is_rejected(
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="not registered"
    ):
...
def test_failure_evidence_rejects_forgery_copy_pickle_and_mutation() -> None:
    ...
    with pytest.raises(publication.Experiment002FinalPublicationError):
    ...
    with pytest.raises(publication.Experiment002FinalPublicationError, match="changed"):
...
def test_every_preexisting_managed_leaf_is_preserved_and_blocks_before_revalidation(
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="not fresh"
    ):
        publication._publish_snapshot(
...
def test_parent_replacement_after_revalidation_is_detected_before_write(
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="directory name changed"
    ):
        publication._publish_snapshot(
            _completed_snapshot(),
            layout,
...
def test_symlinked_parent_is_rejected_without_following_it(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="securely opened"
    ):
        publication._publish_snapshot(
...
def test_link_race_never_overwrites_or_unlinks_concurrent_destination(
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="appeared"
    ):
        publication._publish_snapshot(
            _completed_snapshot(),
            layout,
...
def test_snapshot_rejects_unregistered_report_and_artifact_fields() -> None:
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="identity or status"
    ):
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="unexpected or missing"
    ):
        publication._require_publication_snapshot(
            (*failure[:5], _canonical(failure_document))
        )
...
def test_aggregate_budget_exact_boundary_and_one_byte_overflow(
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="byte budget"
    ):
...
try:
    ...
    except publication.Experiment002FinalPublicationError as error:
...
def test_replaced_temporary_name_cannot_publish_attacker_inode(
    ...
    with pytest.raises(
        publication.Experiment002FinalPublicationError, match="names do not identify"
    ):
        publication._publish_snapshot(
