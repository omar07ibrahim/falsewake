...
CANDIDATE_PATH = ROOT / "src" / "falsewake" / "experiment_003_seed_worker.py"


def _expected_seed_worker_source() -> str:
    source = REFERENCE_PATH.read_text(encoding="utf-8")
    replacements = (
        ("Experiment 002 seed", "Experiment 003 seed"),
        (
        ...
        ("Experiment002SeedWorker", "Experiment003SeedWorker"),
    )
    for old, new in replacements:
        assert old in source
        source = source.replace(old, new)
    coordinator_import = (
        "from falsewake.experiment_003_coordinator import (\n"
        "    REGISTERED_SEEDS,\n"
...
def test_seed_worker_is_the_exact_registered_experiment_003_delta() -> None:
    assert CANDIDATE_PATH.read_text(encoding="utf-8") == (
        _expected_seed_worker_source()
    )


def test_seed_worker_uses_profile_003_activation_and_authority() -> None:
    source = CANDIDATE_PATH.read_text(encoding="utf-8")

    assert "from falsewake.experiment_003_coordinator import (" in source
    assert "from falsewake.experiment_003_run_authority import (" in source
    assert "Experiment003SeedWorkerError" in source
...
def test_seed_worker_inherits_the_registered_scientific_pipeline() -> None:
    ...
    assert "falsewake.experiment_003_registered_" not in source
