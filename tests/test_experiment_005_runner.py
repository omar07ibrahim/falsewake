...
import falsewake.experiment_005_runner as runner

PROJECT_ROOT = Path(__file__).resolve().parents[1]
PROTOCOL_PATH = PROJECT_ROOT / "configs/experiment-005-execution.json"
SOURCE_PATH = PROJECT_ROOT / "src/falsewake/experiment_005_runner.py"
ENTRYPOINT = "src/falsewake/experiment_005_runner.py"
...
DEFAULT_SOURCES = {
    ...
    "src/falsewake/experiment_002_run_authority.py": b"ENGINE_STUB = True\n",
    ...
    "src/falsewake/experiment_005_run_authority.py": (
        b"CAPABILITY = object()\n"
        b"def _verify_and_issue_experiment_005_sealed_child_registration():\n"
        b"    return CAPABILITY\n"
    ),
}
...
def test_runner_is_exact_protocol_normalized_parity() -> None:
    protocol = cast(
        dict[str, Any],
    ...
    parity = cast(
        dict[str, Any],
        cast(dict[str, Any], protocol["execution_delta"])["parity_contract"],
    )
    normalization = cast(dict[str, Any], parity["normalization"])
    encoding = cast(str, normalization["encoding"])
    normalized_text = SOURCE_PATH.read_bytes().decode(encoding)
    ...
    recipe = next(
        ...
        if value["candidate_path"] == "src/falsewake/experiment_005_runner.py"
    )
    reference_path = PROJECT_ROOT / cast(str, recipe["reference_path"])
    reference_bytes = reference_path.read_bytes()

    assert hashlib.sha256(reference_bytes).hexdigest() == recipe["reference_sha256"]
...
def test_main_has_the_exact_fail_closed_sequence_and_no_arguments() -> None:
    ...
    assert _call_name(main.body[5]) == "_claim_permanent_parent_attempt"
    assert _call_name(main.body[6]) == "_run_registered_parent"
...
def test_exact_sequence_rejects_wrong_type_order_extras_and_omissions(
    ...
    with pytest.raises(runner.Experiment005RunnerError, match="exact registered"):
...
def test_clean_import_check_rejects_each_protected_module_root() -> None:
    for name in (
        ...
        with pytest.raises(
            runner.Experiment005RunnerError,
...
def test_clean_import_check_accepts_only_an_exact_tuple_of_names() -> None:
    ...
    with pytest.raises(runner.Experiment005RunnerError, match="snapshot is invalid"):
...
def test_bootstrap_rejects_every_mutable_process_surface(
    ...
    with pytest.raises(runner.Experiment005RunnerError, match=message):
        runner._validate_bootstrap()
...
def test_noncanonical_entrypoint_resolution_fails_closed(
    ...
    with pytest.raises(runner.Experiment005RunnerError, match="resolves outside"):
        runner._validate_bootstrap()
...
def test_authority_import_and_issuance_use_one_fixed_zero_argument_route(
    ...
    authority = ModuleType("falsewake.experiment_005_run_authority")

    def issue(*args: object, **kwargs: object) -> object:
        calls.append(("issue", args, kwargs))
        return capability

    authority.__dict__["verify_and_issue_experiment_005_run_registration"] = issue
    ...
    assert calls == [
        ("falsewake.experiment_005_run_authority", (), {}),
        ("issue", (), {}),
    ]
...
def test_future_coordinator_is_dynamically_imported_and_gets_only_capability(
    ...
    capability = object()
    calls: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
    coordinator = ModuleType("falsewake.experiment_005_coordinator")
    ...
    assert calls == [
        ("falsewake.experiment_005_coordinator", (), {}),
        ("run", (capability,), {}),
    ]
...
def test_attempt_claim_uses_exclusive_creation_and_has_no_delete_route() -> None:
    tree = ast.parse(SOURCE_PATH.read_text(encoding="utf-8"))
    claim = _function(tree, "_claim_permanent_parent_attempt")
    rendered = ast.unparse(claim)
    delete_attributes = {
        node.attr
        for node in ast.walk(claim)
        if isinstance(node, ast.Attribute)
        and node.attr in {"remove", "removedirs", "rmdir", "unlink"}
    }

    assert "os.O_CREAT" in rendered
    assert "os.O_EXCL" in rendered
    assert "os.O_NOFOLLOW" in rendered
    assert delete_attributes == set()


def test_parent_orders_classification_marker_path_authority_and_coordinator(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    events: list[str] = []
    capability = object()
    ...
    def issue() -> object:
        events.append("authority_issue")
        return capability

    def run_registered(registration: object, /) -> None:
        assert registration is capability
    ...
    def import_module(name: str) -> ModuleType:
        if name == runner._AUTHORITY_MODULE:
            events.append("authority_import")
            return authority
        if name == runner._COORDINATOR_MODULE:
            events.append("coordinator_import")
            return coordinator
        raise AssertionError(f"unexpected project import: {name}")
...
def test_permanent_attempt_marker_is_exclusive_and_persistent(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    directory = tmp_path / "attempt-root"
    directory.mkdir(mode=0o700)
    marker = directory / "experiment-005-attempt"
    monkeypatch.setattr(runner, "_ATTEMPT_MARKER_DIRECTORY", os.fspath(directory))
    monkeypatch.setattr(runner, "_ATTEMPT_MARKER_LEAF", marker.name)
    monkeypatch.setattr(runner, "_ATTEMPT_MARKER_PATH", os.fspath(marker))

    runner._claim_permanent_parent_attempt()

    assert marker.read_bytes() == runner._ATTEMPT_MARKER_BYTES
    assert stat.S_IMODE(marker.stat().st_mode) == runner._ATTEMPT_MARKER_FINAL_MODE
    with pytest.raises(
        runner.Experiment005RunnerError,
        match="permanent Experiment 005 attempt is unavailable",
    ):
        runner._claim_permanent_parent_attempt()
    assert marker.read_bytes() == runner._ATTEMPT_MARKER_BYTES


def test_post_creation_failure_never_removes_attempt_marker(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    directory = tmp_path / "attempt-root"
    ...
    def fail_fchmod(descriptor: int, mode: int) -> None:
        del descriptor, mode
        raise OSError(errno.EIO, "synthetic fchmod failure")
    ...
    assert marker.read_bytes() == runner._ATTEMPT_MARKER_BYTES
    assert stat.S_IMODE(marker.stat().st_mode) == runner._ATTEMPT_MARKER_CREATION_MODE


@pytest.mark.parametrize("failed_step", ["validate", "classify", "marker", "parent"])
def test_main_never_advances_past_a_failed_gate(
    ...
    expected = ["validate", "classify", "marker", "parent"]
...
def test_fd7_rejects_wrong_geometry(
    ...
    try:
        if kind == "inheritable":
            os.set_inheritable(descriptor, True)
        monkeypatch.setattr(runner, "_CHILD_SOURCE_BUNDLE_FD", descriptor)
        with pytest.raises(runner.Experiment005RunnerError, match="source bundle"):
            runner._inspect_source_bundle_descriptor(descriptor)
    finally:
        os.close(descriptor)
...
def test_child_bundle_parser_rejects_header_digest_and_frame_tampering(
    ...
    with (
        pytest.raises(runner.Experiment005RunnerError, match=message),
...
def test_child_bundle_parser_rejects_path_traversal(
    ...
    with (
        pytest.raises(runner.Experiment005RunnerError, match="path traversal"),
        _admitted_bundle(monkeypatch, _synthetic_bundle(sources=sources)),
    ):
        pass
...
def test_sealed_finder_loads_exact_origins_and_stable_routes(
    ...
    with _admitted_bundle(monkeypatch) as (descriptor, bundle):
        ...
        with _clean_falsewake_import_surface():
            ...
            authority_engine = importlib.import_module(
...
def test_real_authority_captures_the_exact_sealed_finder_surface(
    ...
    with (
        ...
        authority = importlib.import_module("falsewake.experiment_002_run_authority")

        assert authority._LOADED_SOURCE_ORIGIN_KIND == "sealed_child"
        assert authority._LOADED_SOURCE_PROFILE is authority._EXPERIMENT_005_PROFILE
...
def test_verifier_holds_strong_identity_for_each_import_registry(
    ...
    with (
        ...
        try:
            ...
            with pytest.raises(runner.Experiment005RunnerError):
                finder._verify_sealed_import_state()
        finally:
...
def test_failed_sealed_module_import_cannot_be_retried(
    ...
    sources = dict(DEFAULT_SOURCES)
    sources["src/falsewake/experiment_005_run_authority.py"] = b"not valid python !\n"
    with (
        _admitted_bundle(
            monkeypatch,
            _synthetic_bundle(sources=sources),
...
def test_sealed_finder_supports_nested_exact_source_imports(
    ...
    sources["src/falsewake/experiment_005_run_authority.py"] = (
...
def test_full_synthetic_child_route_uses_only_sealed_modules(
    ...
    try:
        ...
        with _clean_falsewake_import_surface():
            ...
            assert authority.__file__.startswith("falsewake-sealed://experiment-005/")
