...
import falsewake.experiment_003_final_evidence as final_evidence
...
_REPORT_SHA256 = {
    "pass": "d5272b6155349391729aa8fb379086331da016cab3fa419db5a73924b5806d31",
    "gate_failure": "025a7761894f567013e6dcf345218eff69c07f46a63cad521c2a18be955660fd",
}
...
def _profile_003_registration() -> Iterator[None]:
    ...
    snapshot = authority_engine._RepositorySnapshot(
        ...
        profile=authority_engine._EXPERIMENT_003_PROFILE,
    )
...
def test_surface_is_standard_library_only_fixed_and_defaultless() -> None:
    source_path = Path("src/falsewake/experiment_003_final_evidence.py")
    ...
    assert final_evidence.FINAL_REPORT_PATH == "reports/experiment-003-training.json"
...
def test_builder_rejects_cross_profile_and_mismatched_live_registration() -> None:
    ...
    mismatched_registration = issue(
        authority_engine._EXPERIMENT_003_PROFILE,
        head_commit="e" * 40,
    )
    with pytest.raises(
        final_evidence.Experiment003FinalEvidenceError,
        match="differs from the live Experiment 003 capability",
    ):
        final_evidence.build_completed_final_evidence(
...
def test_pass_issues_exact_evidence_artifacts_resources_and_canonical_report() -> None:
    ...
    assert evidence.experiment == "003"
    ...
    protocol = json.loads((ROOT / "configs/experiment-003-execution.json").read_bytes())
    assert report_document["experiment"] == "003"
    assert report_document["scientific_protocol"] == "002"
    assert report_document["predecessor"] == protocol["predecessor"]
...
def test_selector_result_shape_selection_and_process_identity_fail_closed(
    ...
    with pytest.raises((TypeError, final_evidence.Experiment003FinalEvidenceError)):
...
def test_training_role_ordinal_order_and_common_cpu_binding_are_exact() -> None:
    ...
    with pytest.raises(final_evidence.Experiment003FinalEvidenceError):
