...
import falsewake.experiment_006_final_evidence as final_evidence
...
_REPORT_SHA256 = {
    "pass": "546ebe608d5d7361602ef7442f3899317e4014f589363347aa44b406e30fb0b6",
    "gate_failure": "925e0534dfc6ad9e9606ea2446283b4189bd148c4d2335064f2d58eea9bfd313",
}
...
def _profile_006_registration() -> Iterator[None]:
    ...
    snapshot = authority_engine._RepositorySnapshot(
        ...
        profile=authority_engine._EXPERIMENT_006_PROFILE,
    )
...
def test_surface_is_standard_library_only_fixed_and_defaultless() -> None:
    source_path = Path("src/falsewake/experiment_006_final_evidence.py")
    ...
    assert final_evidence.FINAL_REPORT_PATH == "reports/experiment-006-training.json"
...
def test_builder_rejects_cross_profile_and_mismatched_live_registration() -> None:
    ...
    mismatched_registration = issue(
        authority_engine._EXPERIMENT_006_PROFILE,
        head_commit="e" * 40,
    )
    with pytest.raises(
        final_evidence.Experiment006FinalEvidenceError,
        match="differs from the live Experiment 006 capability",
    ):
        final_evidence.build_completed_final_evidence(
...
def test_pass_issues_exact_evidence_artifacts_resources_and_canonical_report() -> None:
    ...
    assert evidence.experiment == "006"
    ...
    assert report_document["experiment"] == "006"
...
def test_selector_result_shape_selection_and_process_identity_fail_closed(
    ...
    with pytest.raises((TypeError, final_evidence.Experiment006FinalEvidenceError)):
...
def test_training_role_ordinal_order_and_common_cpu_binding_are_exact() -> None:
    ...
    with pytest.raises(final_evidence.Experiment006FinalEvidenceError):
