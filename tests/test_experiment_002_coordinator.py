...
def _activation(
    ...
    *,
    ...
    return coordinator._issue_activation(registration, ticket, receipt)
    ...
    if pid == 0:
        try:
            ...
            if child_fd == coordinator._CHILD_CONTROL_FD:
                ...
                os.dup2(child_fd, coordinator._CHILD_CONTROL_FD, inheritable=True)
                child_channel.close()
            activation = coordinator._receive_activation_from_fixed_fd(registration)
            if (
                ...
                os._exit(91)
            coordinator.verify_verified_child_activation(registration, activation)
            os._exit(0)
...
def test_fixed_descriptor_receiver_has_no_fd_or_path_override() -> None:
    signature = inspect.signature(coordinator._receive_activation_from_fixed_fd)
    assert tuple(signature.parameters) == ("registration",)
    source = inspect.getsource(coordinator._receive_activation_from_fixed_fd)
    assert "_CHILD_CONTROL_FD" in source
    assert "fileno=_CHILD_CONTROL_FD" in source
    assert "path" not in source


def test_child_dispatch_has_no_command_path_callback_or_seed_override() -> None:
    signature = inspect.signature(coordinator._run_registered_seed_child)
    parameters = list(signature.parameters.values())
    assert len(parameters) == 1
    assert parameters[0].name == "registration"
    assert parameters[0].kind is inspect.Parameter.POSITIONAL_ONLY
    source = inspect.getsource(coordinator._run_registered_seed_child)
