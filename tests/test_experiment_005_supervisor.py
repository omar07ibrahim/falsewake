...
CANDIDATE_PATH = ROOT / "src/falsewake/experiment_005_supervisor.py"
...
def test_supervisor_is_exact_protocol_normalized_parity() -> None:
    ...
    assert "Experiment 005" in candidate_text
    assert "falsewake-experiment-005-scratch" in candidate_text
    assert "falsewake-experiment-005-staging" in candidate_text
    assert "falsewake-exp005-child-activation" in candidate_text
    assert (
    ...
    assert "Experiment 003" not in candidate_text
    assert "experiment_003" not in candidate_text
    ...
    recipe = next(
        ...
        if value["candidate_path"] == "src/falsewake/experiment_005_supervisor.py"
    )
