...
import falsewake.experiment_002_final_evidence as final_evidence
...
_REPORT_SHA256 = {
    "pass": "e40c50560b3829ff3c2a9d038ac834d90772c0799823558dd7e07b6489ae8773",
    "gate_failure": "9afbf560addfc4a03a0150323c9722d3c32cd933b6ba1b6e6547efc5ef6bcab8",
}
...
def test_surface_is_standard_library_only_fixed_and_defaultless() -> None:
    source_path = Path("src/falsewake/experiment_002_final_evidence.py")
    ...
    assert final_evidence.FINAL_REPORT_PATH == "reports/experiment-002-training.json"
...
def test_pass_issues_exact_evidence_artifacts_resources_and_canonical_report() -> None:
    evidence = final_evidence.build_completed_final_evidence(_selector())
    ...
    assert evidence.experiment == "002"
...
def test_gate_failure_is_completed_evidence_but_omits_selected_model() -> None:
    evidence = final_evidence.build_completed_final_evidence(
        _selector(_unknown_gate_failure_confusion())
    )
...
def test_publication_snapshot_is_ordered_primitive_and_report_last(
    ...
    evidence = final_evidence.build_completed_final_evidence(_selector(confusion))
...
def test_exact_recall_gate_is_recomputed_from_rerun_winner_matrix() -> None:
    evidence = final_evidence.build_completed_final_evidence(
        _selector(_recall_gate_failure_confusion())
    )
...
def test_selector_result_shape_selection_and_process_identity_fail_closed(
    ...
    with pytest.raises((TypeError, final_evidence.Experiment002FinalEvidenceError)):
        final_evidence.build_completed_final_evidence(mutator(_selector()))
...
def test_training_role_ordinal_order_and_common_cpu_binding_are_exact() -> None:
    ...
    with pytest.raises(final_evidence.Experiment002FinalEvidenceError):
        final_evidence.build_completed_final_evidence(
            (tuple(training), selector[1], selector[2])
        )
    ...
    with pytest.raises(
        ...
        final_evidence.build_completed_final_evidence(
            _replace_rerun(selector, different_cpu_rerun)
        )
    ...
    with pytest.raises(
        ...
        final_evidence.build_completed_final_evidence(
            (tuple(parent_pid_training), selector[1], selector[2])
        )
...
def test_malformed_envelope_fails_even_with_coherent_outer_count_and_digest() -> None:
    ...
    with pytest.raises(final_evidence.Experiment002FinalEvidenceError):
        final_evidence.build_completed_final_evidence(
            _replace_rerun(selector, _replace_tuple(rerun, 5, changed))
        )
...
def test_snapshot_counts_digests_and_winner_fields_are_not_trusted(
    ...
    with pytest.raises(final_evidence.Experiment002FinalEvidenceError):
        final_evidence.build_completed_final_evidence(
            _replace_rerun(selector, bad_rerun)
        )
...
def test_rerun_rejects_coherent_history_component_mismatches(
    ...
    with pytest.raises(
        ...
        final_evidence.build_completed_final_evidence(
            _replace_rerun(selector, changed_rerun)
        )
...
def test_every_internal_trust_dataclass_rejects_persistent_descriptor_masking(
    ...
    with pytest.raises(
        ...
        final_evidence.build_completed_final_evidence(_selector())
...
def test_registered_resource_and_uint32_maxima_are_accepted_exactly() -> None:
    ...
    evidence = final_evidence.build_completed_final_evidence(
        (tuple(training), selector[1], rerun)
    )
