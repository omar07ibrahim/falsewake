...
REFERENCE_PATH = ROOT / "src" / "falsewake" / "experiment_004_coordinator.py"
CANDIDATE_PATH = ROOT / "src" / "falsewake" / "experiment_005_coordinator.py"
ATTEMPT_MARKER = Path("/home/ubuntu/gitcode/.t/falsewake-experiment-005-attempt")
PROTOCOL_SHA256 = "824d1677cf8f75567cf1915381f0a9bbc778a74cba879ace80733c3ceb48e54d"
REFERENCE_SHA256 = "6e4df88440c361376ce51a3574a16bbec79ecbc6ff4dd1fcf0cf8be86eac1daa"
...
def test_coordinator_is_the_exact_frozen_experiment_005_parity_delta() -> None:
    ...
    recipe = next(
        item
        for item in recipes
        if item["candidate_path"] == "src/falsewake/experiment_005_coordinator.py"
    )
    assert recipe == {
        "candidate_path": "src/falsewake/experiment_005_coordinator.py",
        "normalization": {
            "global_literals": (
        ...
        "reference_path": "src/falsewake/experiment_004_coordinator.py",
...
def test_coordinator_uses_only_profile_005_control_namespaces() -> None:
    source = CANDIDATE_PATH.read_text(encoding="utf-8")
    ...
    expected_control_bindings = (
        ...
        (
            ...
            '"falsewake.experiment_005_final_publication"'
        ),
    ...
    assert "Experiment004" not in source
    assert "experiment_004" not in source
    assert "experiment-004" not in source
    assert "exp004" not in source
    assert "FW4ACTV1" not in source
    assert "FW4CHLD1" not in source
...
def test_coordinator_retains_the_registered_experiment_002_scientific_domains() -> None:
    ...
    assert "falsewake.experiment_005_process_guard" not in source
    assert "falsewake.experiment_005_child_result" not in source
