...
REFERENCE_PATH = ROOT / "src" / "falsewake" / "experiment_004_seed_worker.py"
CANDIDATE_PATH = ROOT / "src" / "falsewake" / "experiment_005_seed_worker.py"
REFERENCE_SHA256 = "40cc3f0681006b03eac03f4747c60feb7a0ab42c102772afd6b90ef809929361"
...
def test_seed_worker_is_the_exact_frozen_experiment_005_parity_delta() -> None:
    document = _protocol()
    recipes = document["execution_delta"]["parity_contract"]["file_recipes"]
    recipe = next(
        item
        for item in recipes
        if item["candidate_path"] == "src/falsewake/experiment_005_seed_worker.py"
    )
    assert recipe == {
        "candidate_path": "src/falsewake/experiment_005_seed_worker.py",
        "normalization": {
            "global_literals": (
        ...
        "reference_path": "src/falsewake/experiment_004_seed_worker.py",
...
def test_seed_worker_uses_profile_005_activation_and_authority() -> None:
    source = CANDIDATE_PATH.read_text(encoding="utf-8")

    assert "from falsewake.experiment_005_coordinator import (" in source
    assert "from falsewake.experiment_005_run_authority import (" in source
    assert "Experiment005SeedWorkerError" in source
...
def test_seed_worker_inherits_the_registered_scientific_pipeline() -> None:
    ...
    assert "falsewake.experiment_005_registered_" not in source
