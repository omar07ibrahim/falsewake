...
import falsewake.experiment_006_runner as runner

PROJECT_ROOT = Path(__file__).resolve().parents[1]
PROTOCOL_PATH = PROJECT_ROOT / "configs/experiment-006-execution.json"
SOURCE_PATH = PROJECT_ROOT / "src/falsewake/experiment_006_runner.py"
ENTRYPOINT = "src/falsewake/experiment_006_runner.py"
...
DEFAULT_SOURCES = {
    ...
    "src/falsewake/experiment_006_run_authority.py": (
        b"CAPABILITY = object()\n"
        b"def _verify_and_issue_experiment_006_sealed_child_registration():\n"
        b"    return CAPABILITY\n"
    ),
}
...
def test_runner_is_exact_protocol_normalized_parity() -> None:
    ...
    recipe = next(
        ...
        if value["candidate_path"] == "src/falsewake/experiment_006_runner.py"
    )
...
def test_exact_sequence_rejects_wrong_type_order_extras_and_omissions(
    ...
    with pytest.raises(runner.Experiment006RunnerError, match="exact registered"):
...
def test_clean_import_check_rejects_each_protected_module_root() -> None:
    for name in (
        ...
        with pytest.raises(
            runner.Experiment006RunnerError,
...
def test_clean_import_check_accepts_only_an_exact_tuple_of_names() -> None:
    ...
    with pytest.raises(runner.Experiment006RunnerError, match="snapshot is invalid"):
...
def test_bootstrap_rejects_every_mutable_process_surface(
    ...
    with pytest.raises(runner.Experiment006RunnerError, match=message):
        runner._validate_bootstrap()
...
def test_noncanonical_entrypoint_resolution_fails_closed(
    ...
    with pytest.raises(runner.Experiment006RunnerError, match="resolves outside"):
        runner._validate_bootstrap()
...
def test_authority_import_and_issuance_use_one_fixed_zero_argument_route(
    ...
    authority = ModuleType("falsewake.experiment_006_run_authority")

    def issue(*args: object, **kwargs: object) -> object:
        calls.append(("issue", args, kwargs))
        return capability

    authority.__dict__["verify_and_issue_experiment_006_run_registration"] = issue
    ...
    assert calls == [
        ("falsewake.experiment_006_run_authority", (), {}),
        ("issue", (), {}),
    ]
...
def test_future_coordinator_is_dynamically_imported_and_gets_only_capability(
    ...
    capability = object()
    calls: list[tuple[str, tuple[object, ...], dict[str, object]]] = []
    coordinator = ModuleType("falsewake.experiment_006_coordinator")
    ...
    assert calls == [
        ("falsewake.experiment_006_coordinator", (), {}),
        ("run", (capability,), {}),
    ]
...
def test_permanent_attempt_marker_is_exclusive_and_persistent(
    ...
    marker = directory / "experiment-006-attempt"
    ...
    with pytest.raises(
        runner.Experiment006RunnerError,
        match="permanent Experiment 006 attempt is unavailable",
    ):
        runner._claim_permanent_parent_attempt()
    assert marker.read_bytes() == runner._ATTEMPT_MARKER_BYTES
...
def test_fd7_rejects_wrong_geometry(
    ...
    try:
        if kind == "inheritable":
            os.set_inheritable(descriptor, True)
        monkeypatch.setattr(runner, "_CHILD_SOURCE_BUNDLE_FD", descriptor)
        with pytest.raises(runner.Experiment006RunnerError, match="source bundle"):
            runner._inspect_source_bundle_descriptor(descriptor)
    finally:
        os.close(descriptor)
...
def test_child_bundle_parser_rejects_header_digest_and_frame_tampering(
    ...
    with (
        pytest.raises(runner.Experiment006RunnerError, match=message),
...
def test_child_bundle_parser_rejects_path_traversal(
    ...
    with (
        pytest.raises(runner.Experiment006RunnerError, match="path traversal"),
        _admitted_bundle(monkeypatch, _synthetic_bundle(sources=sources)),
    ):
        pass
...
def test_sealed_finder_loads_exact_origins_and_stable_routes(
    ...
    with _admitted_bundle(monkeypatch) as (descriptor, bundle):
        ...
        with _clean_falsewake_import_surface():
            ...
            verify()
            prefix = f"falsewake-sealed://experiment-006/{bundle.source_bundle_sha256}/"
            ...
            assert authority.__file__ == (
                prefix + "src/falsewake/experiment_006_run_authority.py"
            )
            assert coordinator.__file__ == (
                prefix + "src/falsewake/experiment_006_coordinator.py"
            )
            ...
            assert (
                ...
                == DEFAULT_SOURCES["src/falsewake/experiment_006_run_authority.py"]
            )
...
def test_real_authority_captures_the_exact_sealed_finder_surface(
    ...
    with (
        ...
        assert authority._LOADED_SOURCE_PROFILE is authority._EXPERIMENT_006_PROFILE
...
def test_verifier_holds_strong_identity_for_each_import_registry(
    ...
    with (
        ...
        try:
            ...
            with pytest.raises(runner.Experiment006RunnerError):
                finder._verify_sealed_import_state()
        finally:
...
def test_failed_sealed_module_import_cannot_be_retried(
    ...
    sources = dict(DEFAULT_SOURCES)
    sources["src/falsewake/experiment_006_run_authority.py"] = b"not valid python !\n"
    with (
        _admitted_bundle(
            monkeypatch,
            _synthetic_bundle(sources=sources),
...
def test_sealed_finder_supports_nested_exact_source_imports(
    ...
    sources["src/falsewake/experiment_006_run_authority.py"] = (
...
def test_full_synthetic_child_route_uses_only_sealed_modules(
    ...
    try:
        ...
        with _clean_falsewake_import_surface():
            ...
            assert authority.__file__.startswith("falsewake-sealed://experiment-006/")
